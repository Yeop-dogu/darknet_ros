...
void YoloObjectDetector::init() {
  ...
  imageSubscriber_ = imageTransport_.subscribe(
      cameraTopicName, cameraQueueSize, &YoloObjectDetector::cameraCallback,
      this, hints);
  imageSubscriberROS_ =
      nodeHandle_.subscribe(cameraTopicName, cameraQueueSize,
...
void YoloObjectDetector::getImageFromUsb() {
...
void YoloObjectDetector::imageCallback(
    const sensor_msgs::CompressedImageConstPtr &msg) {
  ROS_INFO("[YoloObjectDetector] USB image received.");
  cv::Mat image;
  try {
    image = cv::imdecode(cv::Mat(msg->data),
                         1);  // convert compressed image data to cv::Mat
...
void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr &msg) {
  ...
  try {
    cv::Mat image = cv::imdecode(
        cv::Mat(msg->data), 1);  // convert compressed image data to cv::Mat
    cv::imshow("view", image);
